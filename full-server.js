...
app.post('/api/v1/files/upload', verifyToken, (req, res) => {
  ...
  try {
    ...
    const file = {
      ...
      thumbnailPath: mimeType.startsWith('image/') ? `/thumbnails/${generateId()}.jpg` : null,
      createdAt: new Date().toISOString()
    };
...
app.get('/api/v1/files/:fileId/download', verifyToken, (req, res) => {
  ...
  const hasAccess = conversation?.participants.some(p => p.userId === req.user.userId);
...
app.get('/api/v1/connections', verifyToken, (req, res) => {
  const userConnections = connections.filter(c => 
    c.requesterId === req.user.userId || c.recipientId === req.user.userId
  );
  
  const connectionsList = userConnections.map(conn => {
    const otherUserId = conn.requesterId === req.user.userId ? conn.recipientId : conn.requesterId;
