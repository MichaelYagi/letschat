...
describe('EncryptionService', () => {
  describe('encrypt/decrypt', () => {
    test('should encrypt and decrypt text correctly', () => {
      ...
      const { encrypted, tag } = EncryptionService.encrypt(originalText);
    ...
    test('should produce different outputs for same input', () => {
      ...
      expect(EncryptionService.decrypt(result1.encrypted, result1.tag)).toBe(text);
      expect(EncryptionService.decrypt(result2.encrypted, result2.tag)).toBe(text);
    });
  });
  ...
  describe('asymmetric encryption', () => {
    test('should encrypt and decrypt with RSA key pair', () => {
      ...
      const encrypted = EncryptionService.encryptWithPublicKey(originalText, publicKey);
