...
async function testEncryption() {
  ...
  try {
    ...
    const asymEncrypted = EncryptionService.encryptWithPublicKey(message, publicKey);
    const asymDecrypted = EncryptionService.decryptWithPrivateKey(asymEncrypted, privateKey);
...
async function testSecurity() {
  ...
  try {
    ...
    const validEmail = SecurityService.isValidEmail('user@example.com');
    const invalidEmail = SecurityService.isValidEmail('invalid-email');
...
async function testJWT() {
  ...
  try {
    ...
    console.log(`   Token matches: ${payload.userId === decoded.userId && payload.username === decoded.username}`);
